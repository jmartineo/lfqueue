...
int main(int argc, const char *argv[])
{
  ...
  PrepareBenchmark(&barrier, nprocs);
  printf("===========================================\n");
  printf("  Benchmark: %s\n", argv[0]);
  ...
  int i;
  for (i = 1; i < nprocs; i++) {
    StartBenchmark(&ths[i], thread, bits_join(i, nprocs));
  }

  res[0] = thread(bits_join(0, nprocs));

  for (i = 1; i < nprocs; i++) {
    FinishBenchmark(ths[i], &res[i]);
  }
